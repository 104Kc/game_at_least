...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (registeredUser) {
      router.push("/Each_page_of_the_main_game");
    }
  }, [router]);
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    const registeredUser = localStorage.getItem("registeredUser");
    if (!registeredUser) {
      setError("ยังไม่มีบัญชี กรุณาสมัครสมาชิกก่อน");
      return;
    }

    try {
      const { characterName: savedName, password: savedPassword } = JSON.parse(registeredUser);
      if (savedName !== characterName || savedPassword !== password) {
        setError("ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง");
        return;
      }
    } catch {
      setError("ข้อมูลไม่ถูกต้อง กรุณาสมัครใหม่");
      return;
    }
      ...
      <main className="w-full max-w-md mx-auto px-4">
        <div className="bg-blue-900 bg-opacity-90 rounded-lg shadow-2xl p-8 border-4 border-blue-500">
          ...
          <div className="mt-4 text-center text-sm text-slate-100">
            ยังไม่มีบัญชี?{' '}
            <Link href="/Registration_and_login_page/application" className="text-cyan-300 hover:underline">
              สมัครสมาชิก
            </Link>
          </div>
        </div>
